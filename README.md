...
veloce-hero.mp4    ← local background video (~2.9 MB, 1080p H.264, web-optimized)
README.md          ← this file
```

All three files must ship together when deploying. The HTML references `veloce-hero.mp4` as a relative path, so just keep them in the same folder.
...
The home hero and every secondary page mini-hero both pull from a local file:

```html
<video class="cinema__video" autoplay loop muted playsinline preload="auto"
       src="veloce-hero.mp4">
</video>
```

The bundled `veloce-hero.mp4` is a 1080p H.264 video, web-optimized with `+faststart` (metadata up front so playback starts before the full download), audio stripped, ~2.9 MB. The main hero uses `preload="auto"` for an instant first-paint; the 6 secondary cinema-mini headers on other routes use `preload="metadata"` so they don't fight for bandwidth until the user navigates to them.

To swap in your own video, replace `veloce-hero.mp4` in the same folder (keep the same filename) — or rename and update the seven `src="veloce-hero.mp4"` references in the HTML.
