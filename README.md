...
The home hero pulls from Pexels:

```html
<video class="cinema__video" autoplay loop muted playsinline
       src="https://videos.pexels.com/video-files/30843746/13167197_2560_1440_30fps.mp4">
</video>
```

Replace the `src` with any MP4 URL. Recommended specs:

- 1440p or 2160p resolution for sharpness on large displays
- 30 fps minimum
- 10–30 second loop for seamless playback
- Dark or low-contrast subject so the white text remains legible
- Under 15 MB if you want it inline; larger files should be hosted on a CDN
...
- **Pexels video dependency** — if the Pexels CDN is slow or blocks hot-linking, the hero will fall back to its black background. Self-host the MP4 for production.
...
- **Background video** — Pexels (free for commercial use, no attribution required).
